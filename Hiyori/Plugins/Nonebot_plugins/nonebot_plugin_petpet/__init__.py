...
help_cmd = on_command("头像表情包", aliases={"头像相关表情包", "头像相关表情制作"}, block=True, priority=Priority.低优先级)
block_cmd = on_command("禁用表情", block=True, priority=Priority.低优先级, permission=PERM_EDIT)
unblock_cmd = on_command("启用表情", block=True, priority=Priority.低优先级, permission=PERM_EDIT)
block_cmd_gl = on_command("全局禁用表情", block=True, priority=Priority.低优先级, permission=PERM_GLOBAL)
unblock_cmd_gl = on_command("全局启用表情", block=True, priority=Priority.低优先级, permission=PERM_GLOBAL)
...
def create_matchers():
    ...
    on_message(regex("随机表情"), block=False, priority=Priority.普通优先级).append_handler(random_handler())
