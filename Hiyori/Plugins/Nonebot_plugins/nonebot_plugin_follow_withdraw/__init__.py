...
__plugin_meta__ = PluginMetadata(
    name="同步撤回",
    description="当命令消息被撤回时，妃爱跟随撤回命令结果",
    usage="撤回消息时自动触发",
    config=Config,
    extra={"example": "",
           "Group": "Daily",
...
withdraw_notice = on_notice(rule=check_event, priority=Priority.系统优先级)
clear_cmd = on_command("清除消息记录", priority=Priority.系统优先级, permission=SUPERUSER)
