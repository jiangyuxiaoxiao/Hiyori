...
monitor = on_message(block=False, rule=message_at_rule, priority=Priority.低优先级)
...
who_at_me = on_regex(r"谁.*(@|艾特|圈|[aA][tT])+.?我", priority=Priority.普通优先级)
...
clear_db = on_command("清除数据库", aliases={"clear_db", "db_clear", "已阅"}, priority=Priority.普通优先级)
