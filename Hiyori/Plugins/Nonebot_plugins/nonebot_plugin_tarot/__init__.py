...
divine = on_regex(r"^#?占卜$", priority=Priority.普通优先级)
tarot = on_regex(r"^#?塔罗牌$", priority=Priority.普通优先级)
chain_reply_switch = on_regex(
    pattern=r"^(开启|启用|关闭|禁用)群聊转发(模式)?$", permission=SUPERUSER, priority=Priority.普通优先级, block=True)
